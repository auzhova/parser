...
/**
 * Парсер новостей с сайта https://nemagia.info/
 */
class NemagiaInfoParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'https://nemagia.info';
    ...
    public function parse(): array
    {
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $content = $itemCrawler->filterXPath('//div[@class="content"]');
            $title = $content->filterXPath('//h1[@class="name post-title entry-title"]/span')->text();
            $date = $this->getDate($content->filterXPath('//span[@class="tie-date"]')->text());
            $image = null;
            $imgSrc = $content->filterXPath('//div[@class="entry"]')->filterXPath('//img');
            if ($imgSrc->getNode(0)) {
                $image = $this->getHeadUrl($imgSrc->attr('src'));
            }
            $description = $title;
            $descriptionSrc = $content->filterXPath('//div[@class="entry"]')->filterXPath('//p[@class="article-render__block article-render__block_unstyled"]');
            ...
            $newContentCrawler = $content->filterXPath('//div[@class="entry"]');
            foreach ($newContentCrawler as $contentNew) {
                foreach ($contentNew->childNodes as $childNode) {
                    if ($childNode->nodeName == 'script') {
                        continue;
                    }
                    if ($childNode->firstChild && $childNode->firstChild->nodeName == '#comment') {
                        continue;
                    }
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath('//div[@class="content"]')->filterXPath('//a[@class="more-link"]');
