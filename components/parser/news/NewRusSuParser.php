...
/**
 * Парсер новостей с сайта https://newsrus.su/
 */
class NewRusSuParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'https://newsrus.su';
    ...
    public function parse(): array
    {
        ...
        foreach ($newsList as $newsItem) {
            ...
            $newsCrawler = new Crawler(mb_convert_encoding($contentPage, "utf-8", "windows-1251"));
            $content = $newsCrawler->filterXPath('//div[@id="item_content"]');
            $post->title = $this->clearText($content->filterXPath('//h1')->text());
            $post->description = $post->title;

            $imgSrc = $content->filterXPath('//div[@id="none_image_item_content"]/img');
            if ($imgSrc->getNode(0)) {
                $post->image = $this->getHeadUrl($imgSrc->attr('src'));
            }

            $newContentCrawler = $content->filterXPath('//div[@id="text_suprime_content"]')->children();
