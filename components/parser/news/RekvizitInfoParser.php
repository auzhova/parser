...
/**
 * Парсер новостей с сайта http://rekvizit.info/
 */
class RekvizitInfoParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'http://rekvizit.info/';
    ...
    public function parse(): array
    {
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $title = $itemCrawler->filterXPath('//header[@id="cb-standard-featured bob"]')->filterXPath("//h1")->text();
            $date = $this->getDate($itemCrawler->filterXPath('//header[@id="cb-standard-featured bob"]')->filterXPath('//time')->attr('datetime'));
            $image = null;
            $imgSrc = $itemCrawler->filterXPath('//header[@id="cb-standard-featured bob"]')->filterXPath('//div[@class="cb-mask"]/img');
            if ($imgSrc->getNode(0)) {
            ...
            $newContentCrawler = $itemCrawler->filterXPath("//section")->children();
            foreach ($newContentCrawler as $key => $contentNew) {
                if ($key < 2) {
                    continue;
                }
                if ($contentNew->nodeName == 'div') {
                    $divClawler = new Crawler($contentNew);
                    $img = $divClawler->filterXPath('//img');
                    if (!$img->getNode(0)) {
                        continue;
                    }
                }
                ...
                foreach ($contentNew->childNodes as $childNode) {
                    ...
                    }elseif ($childNode->childNodes->count() && $childNode->nodeName != 'a') {
    ...
    protected function getListNews(string $page): array
    {
        ...
        $main = $crawler->filterXPath("//div[@class='cb-mask-main-img']")->filterXPath("//a");
        if ($main->getNode(0)) {
            $records[] = $main->attr('href');
        }
        $list = $crawler->filterXPath("//div[@class='cb-mask']")->filterXPath("//a");
