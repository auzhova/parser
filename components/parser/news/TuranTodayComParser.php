...
/**
 * Парсер новостей с сайта https://www.turantoday.com/
 */
class TuranTodayComParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'https://www.turantoday.com';
    ...
    public function parse(): array
    {
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $content = $itemCrawler->filterXPath('//div[@id="td-content-main"]');
            $title = $content->filterXPath('//h1[@class="post-title entry-title"]')->text();
            $date = [];
            $dateSrc = $content->filterXPath('//div[@class="date-header"]')->children();
            foreach ($dateSrc as $item) {
                if ($item->nodeName == 'i') {
                    array_push($date, $item->nodeValue . str_replace(['GMT ', ':'], '', $item->getAttribute('title')));//$item->nodeValue . $item->getAttribute('title');
                } else {
                    array_unshift($date, $item->nodeValue);
                }
            }
            $date = implode(' ', $date);
            if (!$date) {
                $date = $content->filterXPath('//div[@class="date-header"]')->text();
            }
            $date = $this->getDate($date);
            $image = null;
            $imgSrc = $content->filterXPath('//div[@class="post-body entry-content"]')->filterXPath('//link[@itemprop="image"]');
            if ($imgSrc->getNode(0)) {
                $image = $this->getHeadUrl($imgSrc->attr('href'));
            }
            $description = '';
            $descriptionSrc = $content->filterXPath('//div[@class="post-body entry-content"]');
            ...
            $newContentCrawler = $content->filterXPath('//div[@class="post-body entry-content"]');
    ...
    protected function setItemPostValue (NewsPost $post, DOMNode $node): void {
        ...
        } elseif ($nodeValue && ($node->parentNode->nodeName == 'blockquote' || $node->parentNode->nodeName == 'quote')) {
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath('//div[@id="td-content-main"]')->filterXPath('//div[@itemprop="articleBody"]/a');
