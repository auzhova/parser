...
/**
 * Парсер новостей с сайта https://www.mvestnik.ru/
 */
class MvestnikRuParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'https://www.mvestnik.ru';
    ...
    public function parse(): array
    {
        ...
        $urlNews = self::SITE_URL . '/newslent/';
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $content = $itemCrawler->filterXPath('//div[@class="article_div"]');
            $title = $content->filterXPath('//h1[@class="article_header"]')->text();
            $date = $this->getDate($content->filterXPath('//span[@class="article_author"]')->text());
            $image = null;
            $imgSrc = $content->filterXPath('//div[@class="article_photo"]')->filterXPath('//img');
            if ($imgSrc->getNode(0)) {
                $image = $this->getHeadUrl($imgSrc->attr('src'));
            }
            $description = $content->filterXPath('//div[@class="articleBody"]/p[1]')->text();
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath('//div[@class="media newsitem_block"]')->filterXPath('//a[@class="media-heading"]');
