...
/**
 * Парсер новостей с сайта http://1m.tv/
 */
class StudioM1TvParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'http://1m.tv';
    ...
    public function parse(): array
    {
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $title = $itemCrawler->filterXPath("//h1[@class='b-title']")->text();
            $date = $this->getDate('now');
            $image = null;
            $description = $itemCrawler->filterXPath("//span[@class='timing']")->text();
            ...
            $newContentCrawler = $itemCrawler->filterXPath('//div[@class="b-flim_body"]')->children();
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath("//div[@class='b-films__item']")->filterXPath("//a[@class='pr_link']");
