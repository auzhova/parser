...
/**
 * Парсер новостей с сайта http://v-n-v.info/
 */
class VnvInfoParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'http://v-n-v.info';
    ...
    public function parse(): array
    {
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $title = $itemCrawler->filterXPath("//h1[@class='title']")->text();
            $dateSrc = $itemCrawler->filterXPath("//span[@class='submitted-by']");
            if ($dateSrc->getNode(0)) {
                $date = $this->getDate($dateSrc->text());
            } else {
                $date = $this->getDate();
            }
            $image = null;
            $imgSrc = $itemCrawler->filterXPath("//div[@class='content clearfix']")->filterXPath('//img');
            if ($imgSrc->getNode(0)) {
                $image = $this->getHeadUrl($imgSrc->attr('src'));
            }

            $galery = $itemCrawler->filterXPath('//div[@class="media-gallery-item"]');
            if ($galery->getNode(0)) {
            ...
            $newContentCrawler = $itemCrawler->filterXPath("//div[@class='content clearfix']")->filterXPath('//div[@class="field-items"]')->children();
            ...
            if ($galery->getNode(0)) {
                $galeryContentCrawler = $galery->filterXPath('//a');
                foreach ($galeryContentCrawler as $galeryContent) {
                    foreach ($galeryContent->childNodes as $childNode) {
                        if ($childNode->childNodes->count()) {
                            foreach ($childNode->childNodes as $childNodeItem) {
                                $this->setItemPostValue($post, $childNodeItem);
                            }
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath("//div[@class='views-field views-field-title']")->filterXPath("//span[@class='field-content']/a");
