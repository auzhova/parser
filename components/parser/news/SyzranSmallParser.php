...
/**
 * Парсер новостей с сайта https://syzran-small.ru/
 */
class SyzranSmallParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'https://syzran-small.ru';
    ...
    public function parse(): array
    {
        ...
        foreach ($newsList as $newsItem) {
            ...
            $content = $newsCrawler->filterXPath('//div[@class="newsbody"]');
            $post->title = $this->clearText($content->filterXPath('//div[@class="newstitle"]/h1')->text());
            ...
            $newContentCrawler = $newsCrawler->filterXPath('//div[@class="js-mediator-article"]');
            ...
            foreach ($newContentCrawler as $contentNew) {
                foreach ($contentNew->childNodes as $childNode) {
                    if ($childNode->nodeName == '#comment' || $childNode->nodeName == 'script') {
                        continue;
                    }
                    $childNodeCrawler = (new Crawler($childNode))->filterXPath('//img[@alt="Дата выхода"]');
                    if ($childNodeCrawler->getNode(0)) {
                        continue;
                    }
    ...
    protected function setItemPostValue (NewsPost $post, DOMNode $node): void {
        ...
        if (in_array($node->nodeName, ['h1', 'h2', 'h3', 'h4', 'h5', 'h6']) && $nodeValue && $nodeValue != $post->title) {
            ...
            &&  strpos($node->getAttribute('href'),'javascript:history.go') === false) {
    ...
    protected function encodeUrl(string $url): string
    {
        ...
        if (isset($partsUrl['host']) && preg_match('/[А-Яа-яЁё]/iu', $partsUrl['host'])) {
    ...
    {
        if (mb_stripos($text, 'Продолжение новости ниже') !== false ||
            mb_stripos($text, 'Новости партнеров') !== false ||
            mb_stripos($text, 'Пожалуйста, выделяйте фразу с опечаткой,') !== false ||
            mb_stripos($text, 'Если вы заметили опечатку,') !== false ||
            mb_stripos($text, 'Поделиться ссылкой:') !== false ||
            mb_stripos($text, 'Вернуться назад') !== false ||
            $text == '/'
        ) {
            return '';
        }
