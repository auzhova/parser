...
/**
 * Парсер новостей с сайта https://mip-info.ru/
 */
class MipInfoRuParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'https://mip-info.ru';
    ...
    public function parse(): array
    {
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $title = $itemCrawler->filterXPath("//div[@class='story-full group']/h1")->text();
            $date = $this->getDate($itemCrawler->filterXPath("//ul[@class='nav-story']/li[1]")->text());
            $image = null;
            $imgSrc = $itemCrawler->filterXPath("//div[@class='story-full group']")->filterXPath('//img');
            if ($imgSrc->getNode(0)) {
                $image = $this->getHeadUrl($imgSrc->attr('src'));
            }
            $description = $itemCrawler->filterXPath("//div[@class='full-str']")->text();
            $start = mb_stripos($description,'MIP-INFO.RU.');
            $description = mb_substr($description, 0, $start+mb_strlen('MIP-INFO.RU.'));
            ...
            $newContentCrawler = $itemCrawler->filterXPath("//div[@class='full-str']");
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath("//div[@id='dle-content']")->filterXPath("//h2/a");
