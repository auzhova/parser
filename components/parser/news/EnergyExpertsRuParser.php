...
/**
 * Парсер новостей с сайта https://www.energy-experts.ru/
 */
class EnergyExpertsRuParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'https://www.energy-experts.ru';
    ...
    public function parse(): array
    {
        ...
        $urlNews = self::SITE_URL . '/news.html';
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $content = $itemCrawler->filterXPath('//div[@class="center"]');
            $title = $content->filterXPath("//h1")->text();
            $date = $this->getDate($content->filterXPath("//div[@class='datenews']")->text());
            $image = null;
            $imgSrc = $content->filterXPath("//td/img");
            if ($imgSrc->getNode(0)) {
                $image = $this->getHeadUrl($imgSrc->attr('src'));
            }

            $description = $content->filterXPath("//div[@id='arttext']")->children();
            if ($description->getNode(0)) {
                $description = $description->getNode(0)->nodeValue;
            }
            ...
            $newContentCrawler = $content->filterXPath("//div[@id='arttext']");
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath("//div[@class='center']")->filterXPath("//a[@class='urlnews']");
