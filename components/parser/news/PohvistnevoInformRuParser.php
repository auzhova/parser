...
/**
 * Парсер новостей с сайта https://pohvistnevo-inform.ru/
 */
class PohvistnevoInformRuParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'https://pohvistnevo-inform.ru';
    ...
    public function parse(): array
    {
        ...
        foreach ($newsList as $news) {
            ...
            $content = $itemCrawler->filterXPath('//div[@class="content_item news_item"]');
            $title = $news['title'];
            ...
            $description = '';
            $descriptionSrc = $content->filterXPath('//div[@class="field ft_html f_content auto_field"]')->children();
            ...
            $newContentCrawler = $content->filterXPath('//div[@class="field ft_html f_content auto_field"]')->children();
            foreach ($newContentCrawler as $contentNew) {
                if ($contentNew->childNodes->count() > 1) {
                    foreach ($contentNew->childNodes as $childNode) {
                        if ($childNode->attributes && $childNode->getAttribute('class') == 'adsman') {
                            continue;
                        }
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath('//div[@id="content_list_news"]')->filterXPath('//div[@class="fields"]');
        foreach ($list as $item) {
            $itemCrawler = new Crawler($item);
            $title = $itemCrawler->filterXPath('//h2[@class="value"]/a')->text();
            $href = $itemCrawler->filterXPath('//h2[@class="value"]/a')->attr('href');
            $date = $itemCrawler->filterXPath('//div[@class="info_bar"]')->filterXPath('//time')->attr('datetime');
            $href = str_replace('#comments', '', $href);
