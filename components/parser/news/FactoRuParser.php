...
/**
 * Парсер новостей с сайта https://facto.ru/
 */
class FactoRuParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'https://facto.ru';
    ...
    public function parse(): array
    {
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $content = $itemCrawler->filterXPath('//div[@class="main-content"]');
            $title = $content->filterXPath('//div[@class="page-header"]/h1')->text();
            $date = $this->getDate($content->filterXPath('//div[@class="date-news"]')->text());
            $image = null;
            $description = '';
            $descriptionSrc = $content->filterXPath('//div[@class="bx_page js-mediator-article"]');
            ...
            $newContentCrawler = $content->filterXPath('//div[@class="bx_page js-mediator-article"]');
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath('//div[@class="news-list-main"]')->filterXPath('//p[@class="name"]/a');
