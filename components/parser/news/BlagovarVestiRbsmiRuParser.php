...
/**
 * Парсер новостей с сайта https://blagovar-vesti.rbsmi.ru/
 */
class BlagovarVestiRbsmiRuParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'https://blagovar-vesti.rbsmi.ru';
    ...
    public function parse(): array
    {
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $content = $itemCrawler->filterXPath('//div[@class="container"]');
            $title = $content->filterXPath('//div[@class="slider2-title"]/h1')->text();
            $date = $this->getDate($content->filterXPath('//div[@class="data"]')->text());
            $image = null;
            $imgSrc = $content->filterXPath('//div[@class="row"]')->filterXPath('//div/img');
            if ($imgSrc->getNode(0)) {
                $image = $this->getHeadUrl($imgSrc->attr('src'));
            }
            $description = '';
            $descriptionSrc = $content->filterXPath('//div[@class="pin"]')->children();
            ...
            $newContentCrawler = $content->filterXPath('//div[@class="pin"]');
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath('//div[@class="news-slider"]')->filterXPath('//h2[@class="news-slider__name"]/a');
