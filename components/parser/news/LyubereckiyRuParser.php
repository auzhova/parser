...
/**
 * Парсер новостей с сайта http://lyubereckiy.ru/
 */
class LyubereckiyRuParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'http://lyubereckiy.ru';
    ...
    public function parse(): array
    {
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $content = $itemCrawler->filterXPath('//div[@id="main-left"]');
            $title = $content->filterXPath('//h2[@class="entry-title"]/span[@class="the_title"]')->text();
            $date = $this->getDate($content->filterXPath('//div[@class="entry-meta row-fluid"]')->text());
            $image = null;
            $imgSrc = $content->filterXPath('//img[@class="attachment-single size-single wp-post-image"]');
            if ($imgSrc->getNode(0)) {
            ...
            foreach ($newContentCrawler as $contentNew) {
                if ($contentNew->childNodes->count() > 1) {
                    foreach ($contentNew->childNodes as $childNode) {
                        if ($childNode->attributes && $attrClass = $childNode->getAttribute('class') == "ratings hreview-aggregate") {
                            continue;
                        }
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath('//div[@id="main-left"]')->filterXPath('//a');
