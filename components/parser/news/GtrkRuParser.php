...
/**
 * Парсер новостей с сайта http://www.gtrk.ru/
 */
class GtrkRuParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'http://www.gtrk.ru/';
    ...
    public function parse(): array
    {
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $title = $itemCrawler->filterXPath('//div[@class="news-single-item"]')->filterXPath("//h2")->text();
            $dateSrc = $itemCrawler->filterXPath('//div[@class="news-single-item"]')->filterXPath("//div[@class='news-single-rightbox']");
            ...
            $image = null;
            $imgSrc = $itemCrawler->filterXPath('//div[@class="news-single-item"]')->filterXPath('//img');
            if ($imgSrc->getNode(0)) {
                $image = $this->getHeadUrl($imgSrc->attr('src'));
            }

            $description = $itemCrawler->filterXPath("//div[@class='news-single-item']/p[1]")->text();
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath("//table[@class='news-list-container']")->filterXPath("//td[@class='news_text']/a");
