...
/**
 * Парсер новостей с сайта http://4pera.com/
 */
class News4PeraComParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'http://4pera.com';
    ...
    public function parse(): array
    {
        ...
        $urlNews = self::SITE_URL . '/rss_mainnews.php';
        ...
        foreach ($newsList as $newsItem) {
            ...
            $imgSrc = $newsCrawler->filterXPath('//div[@class="news-picture"]')->filterXPath('//img[@class="detail_picture"]');
            if ($imgSrc->getNode(0)) {
                $post->image = $this->getHeadUrl($imgSrc->attr('src'));
            }

            $newContentCrawler = $newsCrawler->filterXPath('//div[@class="news-detail"]')->filterXPath("//div[@class='news-text']");
