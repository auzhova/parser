...
/**
 * Парсер новостей с сайта https://os34.ru/
 */
class Os34RuParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'https://os34.ru';
    ...
    public function parse(): array
    {
        ...
        $urlNews = self::SITE_URL . '/articles/';
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $descriptionSrc = $itemCrawler->filterXPath('//div[@class="b-news-detail-body js-news-detail"]');
            foreach ($descriptionSrc as $item) {
                foreach ($item->childNodes as $value) {
                    if ($value->nodeName == 'div' && $value->attributes && $value->getAttribute('class') == 'b-news-detail-body__picture') {
                        continue;
                    }
            ...
            $newContentCrawler = $itemCrawler->filterXPath('//div[@class="b-news-detail-body js-news-detail"]');
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath('//div[@class="l-section__items"]')->filterXPath('//h3[@class="b-section-item__title"]/a');
