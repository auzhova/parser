...
/**
 * Парсер новостей с сайта https://www.crymonline.ru/
 */
class CrymOnlineRuParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'https://www.crymonline.ru';
    ...
    public function parse(): array
    {
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $content = $itemCrawler->filterXPath('//div[@id="top-news"]');
            ...
            $image = null;
            $imgSrc = $content->filterXPath('//div[@class="news-additional-info-block"]')->filterXPath('//a[@rel="preview"]');
            if ($imgSrc->getNode(0)) {
                $image = $this->getHeadUrl($imgSrc->attr('href'));
            } elseif ($imgSrc = $content->filterXPath('//img')) {
                $image = $this->getHeadUrl($imgSrc->attr('src'));
            }
            $description = '';
            $descriptionSrc = $content->filterXPath('//div[@class="news-text"]');
            ...
            $newContentCrawler = $content->filterXPath('//div[@class="news-text"]');
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath('//div[@id="news-center-block"]')->filterXPath('//a[@class="news-title"]');
        ...
        $list = $crawler->filterXPath('//div[@id="news-center-block"]')->filterXPath('//a[@class="news_title"]');
        foreach ($list as $item) {
            $href = $item->getAttribute("href");
            if ($href && !in_array($href, $records)) {
                $records[] = $href;
